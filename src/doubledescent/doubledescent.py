...
def regression(train_path, validation_path):
    ...
    # validation error
    m = x_validation.shape[0]  # number of validation samples
    val_err = (1.0 / (2 * m)) * np.linalg.norm(x_validation @ beta_hat - y_validation) ** 2
    # *** END CODE HERE
    return val_err
    ...
    for lambda_val in reg_list:
        ...
        if lambda_val == 0:
            ...
            if X.shape[0] < X.shape[1]:  # n < d
                K = X @ X.T + lambda_val * np.eye(X.shape[0])
                alpha = np.linalg.solve(K, y)
                beta_hat = X.T @ alpha
            else:
                H = X.T @ X + lambda_val * np.eye(X.shape[1])
                beta_hat = np.linalg.solve(H, X.T @ y)
        
        # validation error
        m = x_validation.shape[0]
        val_err.append((1.0 / (2 * m)) * np.linalg.norm(x_validation @ beta_hat - y_validation) ** 2)
    # *** END CODE HERE
    return val_err
